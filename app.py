...
if not(os.path.isdir('data')):
    os.mkdir('data')
    os.mkdir('data/logs')
utils.initialize(lang=LANG)

with open(f'resources/questionnaire/questions_{LANG}.txt', 'r', encoding='utf-8') as f:
...
def get():
    return render_template(f"index_{LANG}.html", time=datetime.now().strftime('%H:%M'))
...
def post_question():
    ...
    if not os.path.isfile(memory_file_path):
        ...
        if LANG == 'ja':
            timeout_message = "タイムアウトしました．もう一度最初からやり直してください．"
        else:
            timeout_message = "The session has timed out. Please start over from the beginning."
        response = html_builder.guide_bubble(timeout_message)
    ...
    if int(ques_number) >= MAX_QUESTIONS or END_TOKEN in response:
        ...
        if LANG == 'ja':
            ...
            response = "The interview is now finished! The AI is writing a review based on our conversation. Please wait a moment..."
...
def terminate_interview():
    ...
    if LANG == 'ja':
        ...
        finish_text = "The review has been generated! Below is the product review written by the AI based on our conversation."
